...
class Project_model extends CI_Model {
	...
	public function findProjectAll($where = array(), $whereOR = array(), $whereLike = array())
	{
		$this->filter($where = array(), $whereOR = array(), $whereLike = array());

		$this->db->select('*, (SELECT COUNT(id_employee) FROM employee_project WHERE id_project = project.id_project) as team');
		$this->db->join('organization_item', 'organization_item.id_organization_item = project.id_organization_item');
		$this->db->join('mas_project_status as status', 'status.id_mas_project_status = project.id_mas_project_status');
	...
	public function memberProject($id_employee = 0, $resType = 'count', $where = array(), $whereLike = array())
	{
		...
		if($resType == 'count') {
			...
			$this->db->join('organization_item', 'organization_item.id_organization_item = project.id_organization_item');
			$this->db->join('mas_project_status as status', 'status.id_mas_project_status = project.id_mas_project_status');
			$this->db->join('mas_employee_project_position emp_p', 'emp_p.id_mas_employee_project_position = employee_project.id_mas_employee_project_position');
