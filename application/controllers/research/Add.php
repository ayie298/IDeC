	...
	public function index()
	{
		if($_POST) {
			...
			$data = array(
				...
				'year' => date('Y'),
				'research_team' => $this->input->post('penyusun')
			);
			...
			$dataPemanfaatan = array('id_research' => $id, 'research_pemanfaatan' => $this->input->post('pemanfaatan'));

			$this->db->insert('research_utilization', $dataPemanfaatan);

			$this->session->set_flashdata('notice', 'Data berhasil disimpan');
			
			redirect('research/edit/' . md5($id));
			} else {
		...
		$data['type']		  = $this->idec_model->findMaster('mas_research_type');

		$this->template->write_view('content', MODULE_VIEW_PATH.'research/research_add', $data);
